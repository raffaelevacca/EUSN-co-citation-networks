...
names <- names[!(names %in% c("li", "yu", "wang", "ma", "van", "lin", "liu", "zhang", "chen", "zhou", "tang", "huang", "jung"))]
...
# Save
computer.bi <- bi.author.gr
computer.one <- author.gr
save(list=ls(pattern=".bi|.one"), file= "./Data/graphs.13.rda")
...
png.def("./Figures/computer.png", width= 1000, height= 1000)
plot.gr(gr, layout= layout, vertex.size=v.size, edge.width= width, edge.color= e.col, vert.col= v.col, vert.frame.col= NA, labels= labels, label.color="darkblue", label.cex = l.cex)
dev.off()
...
## Examples:
# citations[grep("^zhou\\>", citations[[1]]),]
# citations[grep("^lu\\>", citations[[1]]),]
# citations[grep("^fu\\>", citations[[1]]),]
# citations[grep("^wang\\>", citations[[1]]),]
# citations[grep("^hu\\>", citations[[1]]),]
# citations[grep("^tang\\>", citations[[1]]),]
# citations[grep("^liu\\>", citations[[1]]),]
# citations[grep("^cohen\\>", citations[[1]]),]
## Remove very common last names
names <- names[!(names %in% c("zhou", "lu", "fu", "wang", "van", "hu", "tang", "liu", "cohen"))]
# Other last names may seem very common but in fact are not
# Example:
# citations[grep("^hui\\>", citations[[1]]),]
# citations[grep("^shi\\>", citations[[1]]),]
...
# Save
physics.bi <- bi.author.gr
physics.one <- author.gr
save(list=ls(pattern=".bi|.one"), file= "./Data/graphs.13.rda")
...
# Plot
png.def("./Figures/physics.png", width= 1000, height= 1000)
plot.gr(gr, layout= layout, vertex.size=v.size, edge.width= width, edge.color= e.col, vert.col= v.col, vert.frame.col= NA, labels= labels, label.color="darkblue", label.cex = l.cex)
dev.off()
